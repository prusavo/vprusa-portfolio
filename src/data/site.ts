...
export const media = {
  ...
  /**
   * The Samoyed. Null until the generated photo lands — the page shows an
   * honest "photo pending" plate meanwhile. When it arrives: run it through
   * the same duotone pipeline as the portrait (docs/asset-prompts.md), save as
   * /media/dog.webp and fill this in.
   */
  dog: null as { src: string; alt: string; width: number; height: number } | null,
} as const;
...
export const about = {
  story: [
    ...
    'Off the clock: running, the gym, and romantasy novels — yes, the kind with fae courts and terrible decisions; no, I regret nothing. Whatever time is left belongs to a Samoyed who is technically a cloud.',
  ],
} as const;
